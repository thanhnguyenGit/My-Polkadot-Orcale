...
pub mod pallet {
    ...
    use frame_system::{
        self as system,
        pallet_prelude::*,
        offchain::{SubmitTransaction, SendUnsignedTransaction},
    };
    ...
    #[derive(Encode, Decode, Clone, PartialEq, MaxEncodedLen,Eq,RuntimeDebug, scale_info::TypeInfo)]
    pub struct CatFact {
    ...
    pub enum Event<T: Config> {
        NetworkRunning {msg: String},
        AddData { who: Option<T::AccountId>, data: CatFact },
        FetchedData { url: String, data: BoundedVec<u8,ConstU32<128>> },
        UnsignedTxCommit { who: Option<T::AccountId> },
        GenesisEvent {
            init_value: u32
        },
    }
    ...
    impl<T: Config> ValidateUnsigned for Pallet<T> {
        ...
        fn validate_unsigned(_source: TransactionSource, call: &Self::Call) -> TransactionValidity {
            ...
            match call {
                Call::submit_unsigned_tx { block_number, catfact } => {
                    let current_block = <system::Pallet<T>>::block_number();
                    if *block_number > current_block {
                        return InvalidTransaction::Future.into();
                    }
                    if catfact.fact.is_empty() || catfact.length == 0 {
                        log::info!("Validating Fail");
                        return InvalidTransaction::Custom(1).into();
                    }
                    ValidTransaction::with_tag_prefix("Oracle")
                        .priority(T::UnsignedPriority::get())
                        .longevity(TransactionLongevity::try_from(T::GracePeriod::get().into()).unwrap())
                        .propagate(true)
                        .build()
                }
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn debig_emit_msg_event(origin: OriginFor<T>) -> DispatchResult {
            ...
            Self::deposit_event(Event::<T>::NetworkRunning {
                msg
            });
            Ok(())
        }
        ...
        pub fn get_init_value(orign: OriginFor<T>) -> DispatchResult {
            let init_value = InitValue::<T>::get();
            Self::deposit_event(Event::<T>::GenesisEvent {
                init_value
            });
            Ok(())
        }
    ...
    impl<T: Config> Pallet<T> {
        ...
        fn ocw_do_fetch_data() -> Result<CatFact, http::Error> {
            ...
            let pending = request.deadline(ttl).send().map_err(|_| http::Error::IoError)?;
            let response = pending.try_wait(ttl).map_err(|_| http::Error::DeadlineReached)??;
        ...
        fn parse_response_to_json(response: &str) -> Option<CatFact> {
            ...
            if let JsonValue::Object(obj) = val {
                ...
                let fact_vec = BoundedVec::<u8, ConstU32<128>>::try_from(fact_bytes).map_err(|invalid| {
                    log::warn!("Cat fact {:#?} is too long: {} bytes",invalid, fact.len());
                    ()
                }).ok()?;
                Some(CatFact { fact: fact_vec, length })
            } else {
                None
            }
        }
        ...
        fn add_cat_fact(caller: Option<T::AccountId>, new_cat_fact: CatFact) {
            ...
            CatFactStorage::<T>::mutate(|cat_facts| {
                ...
                cat_facts.try_push(new_cat_fact.clone()).expect("Storage overflow");
            });
